...
export function applyEloResult({
  ...
  const winnerDelta =
    kFactor(winnerComparisonCount) * weight * (1 - winnerExpected);
  const loserDelta =
    kFactor(loserComparisonCount) * weight * (0 - loserExpected);
...
export function confidenceFromComparisons(comparisonCount: number) {
